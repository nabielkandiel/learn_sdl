...
class ResetButton : public ObjectBase
{
  private:
    ...
    static constexpr size_t TOTAL_DIRS =
        static_cast<size_t>(ButtonSprite::COUNT);
  ...
  public:
    ...
    ResetButton(ResetButton &&) = delete;
    ResetButton &operator=(const ResetButton &) = delete;
    ResetButton &operator=(ResetButton &&) = delete;

    ResetButton(GameContext &context);
    ~ResetButton() override;
